  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 3 }}>
        {c.release_date && (
          <span style={{ color: "var(--text-muted)", fontSize: 9, opacity: 0.55 }}>{c.release_date.slice(0, 4)}</span>
        )}
        <a
          href={`https://open.spotify.com/album/${c.spotify_id}`}
          target="_blank"
          rel="noopener noreferrer"
          onClick={(e) => e.stopPropagation()}
          style={{ color: "#1DB954", lineHeight: 1, marginLeft: "auto" }}
          title="Spotify에서 확인"
        >
