...
export default function StoryCardPreviewModal({
  ...
  // 캡처 전용 ref — portal로 document.body에 마운트, transform/overflow 부모 전혀 없음
  ...
  const handleSave = async () => {
    if (!captureCardRef.current || capturing) return;
    setCapturing(true);
    try {
  ...
  return (
    <>
      {/* 캡처 전용 카드 — document.body portal, position:fixed top:0 left:0
          transform/overflow 부모 완전 없음. zIndex:-1로 시각적으로 숨김.
          opacity는 절대 사용 금지 (html2canvas가 투명하게 캡처함) */}
      {isMounted && createPortal(
        ...
        >
          <StoryCard containerRef={captureCardRef} {...cardProps} />
        </div>,
        document.body
      )}
