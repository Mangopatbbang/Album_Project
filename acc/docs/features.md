...
아차청음사는 크게 **5개의 메인 탭**으로 구성된다:

| 경로 | 이름 | 역할 |
|------|------|------|
| `/` | 홈 | 통계 + 오늘의 인연 + 이번 주 청음 + 갑론을박 플립카드 |
| `/albums` | 음반고 | 전체 앨범 목록 — 검색·필터·추가 |
| `/best` | 청음감 | 평점 기반 랭킹 — 통합·연도·장르·아티스트 |
| `/community` | 청음방 | 멤버 현황 + 청음 소감 모음 |
| `/profile/[userId]` | 청음록 | 개인 청음 기록 (취향·청음기·소셜 탭) |

그 외 `/reviews`(청음평), `/themes`(청음집), `/members`(청음인), `/board`(게시판), `/admin`(어드민)이 있다.

**앨범 모달**은 서비스 전반에 걸쳐 사용되는 핵심 UI다.  
어느 페이지에서든 앨범 클릭 시 열리며, 트랙리스트·점수·한줄평·멤버 평가 등 앨범에 관한 모든 정보를 담는다.

---

## 홈 `/`

서비스 진입 화면. 통계 수치로 서비스 규모를 보여주고, 오늘의 인연·이번 주 청음·갑론을박으로 청음사 현황을 한눈에 전달한다.  
...
| 이번 주 청음 | `HomeWeeklySection.tsx` | 최근 7일 이내 등록된 앨범 가로 스크롤 목록 |
| 갑론을박 / 나중에 들을 앨범 | `HomeFlipCard.tsx` | 점수 분산 1위 앨범 목록 ↔ 내 위시리스트 플립 전환. 버튼 클릭 시 3D rotateY 애니메이션 (160ms×2단계). 비로그인 시 로그인 유도 메시지 |
...
| SoundCloud 링크 | soundcloud_url 등록된 앨범에 외부 링크 버튼 표시 |
...
멤버 현황과 청음 소감을 한눈에 볼 수 있는 공간. 청음사 구성원을 소개하고 멤버들의 한줄 소감을 모아 보여준다.  
2026-07 IA 개편 시 "커뮤니티"에서 "청음방"으로 이름 변경됐다.

| 기능 | 컴포넌트 / 파일 | 설명 |
|------|----------------|------|
| 멤버 현황 | `community/page.tsx` | 등록된 멤버 목록 (아바타·표시명·통계) |
| 청음 소감 모음 | `community/page.tsx` | 멤버들의 최근 한줄평 피드 |
...
**취향·청음기·소셜** 3개 탭으로 구성되며, 통계 수치부터 연간 히트맵, 팔로우 소셜 피드까지 가장 많은 섹션을 가진 페이지다.  
프로필 스냅샷 카드를 이미지로 다운로드해 SNS에 공유할 수 있다.

**탭 구조:**
- **취향 탭**: 통계·점수 분포·장르·아티스트·명반전·찜 목록·인사이트
- **청음기 탭** (구: 활동): 연간 히트맵·최근 청음·한줄평·인연 앨범·캘린더·청음 로그
- **소셜 탭**: 팔로우 카운트·팔로우 피드·취향 비교
...
| 찜 목록 | `WatchlistSection.tsx` | 찜한 앨범 목록 (취향 탭) |
| 연간 청음 히트맵 | `AnnualHeatmap.tsx` | GitHub식 365일 격자 — 평가 날짜별 빈도 시각화 (청음기 탭) |
| 좋아하는 곡 | `LikedTracksButton.tsx` | liked_tracks 앨범 목록 모달 |
| 팔로우 카운트 & 버튼 | `FollowButton.tsx` | 팔로워·팔로잉 수 표시 + 팔로우/언팔로우 토글 (소셜 탭) |
| 팔로우 피드 | `SocialFeed.tsx` | 내가 팔로우한 멤버들의 최근 평가 목록 (소셜 탭, 본인 프로필 한정) |
| 취향 비교 | `ComparisonSection.tsx` | 다른 멤버와의 공통 청음 + 점수 차이 (소셜 탭) |
